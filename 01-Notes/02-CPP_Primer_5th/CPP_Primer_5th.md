...
template <typename T, typename... Args>//模板参数
void foo(const T &t, const Args& ... rest);//函数参数
```
...
#### 16.4.1 Writing a Variadic Function Template 编写可变参数函数模板
...
注：当定义可变参数版本的print时，非可变参数版本的声明必须在作用域中。否则可变参数版本会无限递归。

#### 16.4.2 Pack Expansion 包拓展
...
#### 16.4.3 Forwarding Parameter Packs 转发参数包
...
> **c++ 中转发参数包是如何实现的**
>
> 在 C++ 中，可以使用可变参数模板来实现转发参数包。这是一种用于函数模板的 C++ 技术，允许将参数包转发到另一个函数的过程。
>
> 例如，假设你有一个函数 `logger`，它接受一个格式字符串和一个可变参数列表，并将这些参数输出到日志文件中。你可能希望能够将这些参数包转发到另一个函数，例如 `printf`，以便在屏幕上显示日志信息。
>
> 为了实现这一点，你可以使用可变参数模板，如下所示：
>
> ```
> Copy codetemplate<typename... Args>
> void logger(const char* format, Args... args) {
>   // 将参数包转发到 printf
>   printf(format, args...);
> 
>   // 将参数包转发到另一个函数
...
> ```
>
> 在这里，`Args...` 定义了一个可变参数模板，表示可以接受任意数量的参数。在函数体中，可以使用 `args...` 将这些参数包转发到其他函数。
>
> 希望这能帮到你。
>
> 
>
> **如果在此基础上使用forward机制一起编写函数，给出你的建议和示例**
>
> 使用 `std::forward` 可以使函数模板更加通用，因为它可以保留实参的类型信息。这在转发参数包时特别有用，因为它可以保证转发的参数包的类型与原始参数包相同。
>
> 例如，假设你有一个函数模板 `wrapper`，它接受一个可调用对象和一个参数包，并将该参数包转发到可调用对象。为了保留实参的类型信息，你可以使用 `std::forward`，如下所示：
>
> ```
> Copy codetemplate<typename F, typename... Args>
> auto wrapper(F&& f, Args&&... args) {
>   // 使用 std::forward 转发参数包
>   return std::forward<F>(f)(std::forward<Args>(args)...);
> }
> ```
>
> 在这里，`std::forward<F>(f)` 转发了可调用对象 `f`，而 `std::forward<Args>(args)...` 则转发了参数包 `args`。
>
> 希望这能帮到你。
>
> 
>
> **为什么wrapper 的参数列表都是右值引用？**
>
> 在函数模板 `wrapper` 中，将所有参数都声明为右值引用有两个原因：
>
> 1. 右值引用可以绑定到右值，因此可以避免拷贝。这对于函数模板来说非常重要，因为参数可能是任意类型，包括大型对象。使用右值引用可以避免不必要的拷贝，从而节省内存和时间。
> 2. 右值引用可以保留实参的类型信息。这对于函数模板来说非常重要，因为它可以使函数模板更加通用。例如，在上面的示例中，如果 `F` 和 `Args` 都声明为普通引用，那么传递给 `wrapper` 的可调用对象和参数包都将被转换为普通引用。这会丢失实参的类型信息，使得函数模板不能正确地转发参数包。
>
> 希望这能帮到你。
...
### Chapter Summary
 ...
 模板是C++语言与众不同的特性，也是标准库的基础。一个模板就是一个编译器用来生成特定类类型或函数的蓝图。生成特定类或函数的过程称为实例化。我们只编写一次模板，就可以将其运用于多种类型和值，编译器会为每种类型和值进行模板实例化。
...
| 显式实例化   | explicit instantiation      | 一个声明，为所有模板参数提供了显式实参。用来指导实例化过程。如果声明是extern的。模板将不会被实例化 |
